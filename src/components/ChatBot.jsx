...
const ALL_MODELS = AI_PROVIDERS.flatMap(p => p.models.map(m => ({ ...m, provider: p.name, providerIcon: p.icon })));
                  ...
                  <div className="absolute top-full left-0 mt-2 w-[520px] bg-[#1a1725] border border-white/[0.08] rounded-2xl shadow-2xl z-50 flex overflow-hidden max-h-[480px]">
                    ...
                    <div className="flex-1 overflow-y-auto p-2">
                      {selectedProvider.models.map(model => (
                        ...
                        >
                          <span className="text-gray-500">{selectedProvider.icon}</span>
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full max-w-5xl">
              {ALL_MODELS.slice(0, 15).map(model => (
                ...
                >
                  <span className="text-gray-500 text-lg shrink-0">{model.providerIcon}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
